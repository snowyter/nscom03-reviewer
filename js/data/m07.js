  ...
  sections: [
    {
      ...
      body: [
        ...
        { type: "note", text: `Exam framing: the three families are not ranked by quality. They are ranked by assumptions. Contention is cheap when traffic is bursty and the load is light; controlled access is stable when the load is heavy and stations are few; channelization is best when traffic is continuous and predictable.` }
      ]
    },
    {
      id: "s2",
    ...
    },
    {
      ...
      body: [
        ...
        { type: "h3", text: "Slotted ALOHA halves the window" },
        { type: "p", text: `Slotted ALOHA repairs half the damage by imposing a clock. Time is divided into intervals whose length equals one frame time, and a station may begin transmitting only at the beginning of a slot. Frames therefore either overlap completely or not at all — partial overlap is impossible. That single restriction cuts the vulnerable time in half, from 2 × Tfr to 1 × Tfr, because a competitor can no longer start a frame one frame time before A and catch A's beginning; the earliest it can start is the boundary of A's own slot.` },
        { type: "fig", fig: "m07-p10-frames-in-slotted-aloha" },
        { type: "fig", fig: "m07-p11-vulnerable-time-in-slotted-aloha" },
        {
          type: "formula",
        ...
        { type: "note", text: `The two numbers to memorize are 18.4 percent for pure ALOHA at G = 0.5 and 36.8 percent for slotted ALOHA at G = 1. Slotted is exactly double, and the reason is that its vulnerable time is exactly half.` }
      ]
    },
    {
      id: "s4",
      ...
      body: [
        { type: "p", text: `Carrier sense multiple access requires that each station first listen to the medium — check the state of the medium — before sending. Instead of transmitting blindly as ALOHA does, a CSMA station asks the physical layer whether the channel is busy. If the channel is idle, it proceeds; if not, the protocol's persistence rule tells it what to do with the waiting time. The name decomposes neatly: carrier sense means listening before transmitting, multiple access means several stations share the same channel, and the persistence rule is the add-on that makes the scheme usable.` },
        { type: "p", text: `Carrier sense reduces the possibility of a collision, but it cannot eliminate it. The possibility survives because of propagation delay: when a station sends a frame, it still takes time — although very short — for the first bit to reach every station and for every station to sense it. A station far from the transmitter may sample the medium a microsecond before that first bit arrives, find the channel apparently idle, and begin its own transmission. Both signals then arrive at the far end together and collide.` },
        { type: "fig", fig: "m07-p12-carrier-sense-multiple-access-csma" },
        { type: "fig", fig: "m07-p13-vulnerable-time-in-csma" },
        { type: "p", text: `This is why the vulnerable time in CSMA is one propagation delay Tp rather than a frame time. The window during which a second station can wrongly conclude the medium is free equals the time it takes the first bit to cross the medium — the propagation delay. Shorten the medium and the window shrinks; the CSMA vulnerable time has nothing to do with how long the frame is.` },
        {
          type: "formula",
        ...
        { type: "h3", text: "The three persistence methods" },
        { type: "p", text: `When a station finds the channel busy, the persistence method decides what it does next. The channel has just been sampled, so the station must decide whether to sample again immediately, sample again after a random wait, or sample on a probability. Each choice trades collision rate against idle time, and the names come directly from those choices.` },
        { type: "list", items: [
        ...
        { type: "fig", fig: "m07-p14-csma-persistence-methods" },
        { type: "fig", fig: "m07-p15-csma-persistent-flow" },
        { type: "h3", text: "Comparing the three persistence methods" },
        { type: "table", head: ["Method", "Behavior when channel busy", "Behavior when channel goes idle", "Chief weakness"], rows: [
    ...
    },
    {
      ...
      body: [
        ...
        { type: "fig", fig: "m07-p20-carrier-sense-multiple-access-collis" },
        { type: "fig", fig: "m07-p21-flow-diagram-csma-ca" },
        { type: "h3", text: "The procedure, step by step" },
        { type: "p", text: `The flow diagram on the slide spells out the logic in a chain of questions. The station asks whether the channel is idle; if it is, it still does not transmit — it waits one IFS. After the IFS, it checks again. Only if the channel is still idle does it enter the contention window, pick a random number of slots, and wait that long. Having waited the chosen number of slots with every one of them idle, it transmits and then waits for a time-out period for the acknowledgment.` },
        { type: "list", items: [
    ...
    },
    {
      ...
      body: [
        { type: "p", text: `The hidden node problem, also called the hidden terminal problem or hidden station problem, occurs when a node can communicate with a wireless access point (AP) but cannot directly communicate with other nodes that are communicating with that same AP. In a typical arrangement, station A and station C are both in range of the AP, but A is out of range of C and C is out of range of A, because some obstacle or merely distance separates them.` },
        { type: "p", text: `This leads to difficulties in medium access control, because multiple nodes can send data packets to the AP simultaneously, which creates interference at the AP and results in no packet getting through. The crux is that carrier sensing fails here through no fault of the algorithm. Station A senses the medium before transmitting, hears nothing, and correctly concludes from the evidence available to it that the channel is idle. It has no way to know that C is already transmitting, because C's signal never reaches it. The collision happens at the AP, not at the sender, so the sender's sensing was accurate yet useless.` },
        { type: "fig", fig: "m07-p25-hidden-node-problem" },
        { type: "p", text: `The solution is to use the RTS and CTS handshake frames, and the beauty of the solution is that the CTS travels the other direction. Station A sends an RTS to the AP; the AP replies with a CTS that is broadcast and heard by every station in range of the AP — including station C. Because C hears the CTS and cannot hear the RTS, C learns about the reservation from the reply rather than from the request. The hidden station is not hidden from the AP, and the CTS is sent by the AP, so hiding is neutralized.` },
        { type: "table", head: ["Aspect", "Hidden node", "Exposed node"], rows: [
    ...
    },
    {
      ...
      body: [
        ...
        { type: "h3", text: "Polling" },
        { type: "p", text: `Polling works with topologies in which one device is designated as a primary station and the other devices are secondary stations. The primary device controls the link; the secondary devices follow its instructions. This method uses poll and select functions to prevent collisions, and the primary's failure takes the whole system down — the drawback the slide names explicitly.` },
        { type: "list", items: [
    ...
    },
    {
      ...
      body: [
        ...
        { type: "fig", fig: "m07-p39-digital-signal-created-by-four-stati" },
        { type: "fig", fig: "m07-p40-decoding-of-the-composite-signal-for" },
        {
          type: "example",
        ...
        { type: "note", text: `The elegance of CDMA is that the decoder needs no schedule, no frequency allocation, and no coordination — only the target station's chip sequence. Orthogonality does all the work, which is why CDMA scales gracefully when many stations transmit continuously.` }
      ]
    },
    {
      id: "s12",
      ...
      body: [
        ...
        { type: "h3", text: "The four numbers worth memorizing cold" },
        { type: "list", items: [
        ...
        { type: "p", text: `Two derivations recur in examinations and are worth being able to reproduce rather than recall. The first is the ALOHA vulnerable-time argument: it is a purely geometric statement about which competitor start times overlap the frame, and it yields the exponents −2G and −G once you assume Poisson arrivals. The second is the CSMA/CD minimum-frame argument: it is a round-trip timing statement, and it yields the 64-byte Ethernet minimum as a necessary condition for collision detection to be possible at all.` },
        { type: "note", text: `A compact way to remember the whole module: ALOHA talks without listening (18.4 percent), slotted ALOHA talks without listening but on a clock (36.8 percent), CSMA listens before talking, CSMA/CD listens while talking, CSMA/CA asks permission before talking, and channelization never has to talk about it because the channel was divided in advance.` }
      ]
    }
  ],
  flashcards: [
    { q: "Why is a shared link called a multi-point or broadcast link, and what new problem does that create?", a: "Because every transmission propagates to all attached stations. The new problem is medium access control: deciding which station may transmit so that two do not transmit at once and destroy each other's frames.", sec: "s1" },
    { q: "Which sublayer of the data-link layer handles access to the shared medium?", a: "The media access control (MAC) sublayer, which sits between logical link control above and the physical layer below.", sec: "s1" },
    { q: "What is the vulnerable time of pure ALOHA, and why is it that length?", a: "It is 2 × Tfr, two frame times. A frame is ruined by any competitor starting from one frame time before to one frame time after its own start, because pure ALOHA never senses the channel.", sec: "s3" },
    { q: "What is the vulnerable time of slotted ALOHA?", a: "1 × Tfr, one frame time. Transmissions can start only on slot boundaries, so partial overlap is impossible and a frame is ruined only by another frame in the same slot.", sec: "s3" },
    { q: "State the maximum throughput of pure and slotted ALOHA with the load at which each occurs.", a: "Pure ALOHA peaks at 1/(2e) ≈ 18.4 percent when G = 0.5. Slotted ALOHA peaks at 1/e ≈ 36.8 percent when G = 1. Slotted is exactly double.", sec: "s3" },
    { q: "What does carrier sense mean in CSMA, and why can it not eliminate collisions?", a: "It means each station listens to the medium before sending. Collisions survive because of propagation delay: the first bit takes time to reach every station, so a far station can sample the channel, find it apparently idle, and transmit just before the first bit arrives.", sec: "s4" },
    { q: "Describe 1-persistent, non-persistent and p-persistent CSMA.", a: "1-persistent senses continuously and transmits immediately when the medium is idle. Non-persistent waits a random time then senses again and transmits if idle. p-persistent transmits with probability p in each idle slot and defers with probability 1 − p.", sec: "s4" },
    { q: "What is the vulnerable time of CSMA, and what ratio should be kept small?", a: "It is Tp, one propagation delay. The ratio rho = Tp / Tfr should be kept at about 0.01 or less so the window is a negligible fraction of a frame.", sec: "s4" },
    { q: "What does CSMA/CD add to CSMA, and how does a station know a collision happened?", a: "It adds collision detection: the station keeps listening to the medium while it transmits and compares what it hears with what it sent. A mismatch means another station's signal is present, so the frame is aborted and sent again.", sec: "s5" },
    { q: "State the binary exponential backoff rule for CSMA/CD.", a: "After n collisions choose a random integer k uniformly from 0 through 2^n − 1 and wait k slot times. The window doubles with each collision, capped at n = 10 (0 through 1023), and the frame is dropped after 16 collisions.", sec: "s5" },
    { q: "Why does Ethernet impose a 64-byte minimum frame size?", a: "A frame must still be in progress after one round-trip propagation time, 2 × Tp, so the sender is still listening when a collision can return. At 10 Mbps with 2 × Tp = 51.2 microseconds, that gives 512 bits = 64 bytes.", sec: "s5" },
    { q: "Name CSMA/CA's three collision-avoidance strategies.", a: "The interframe space (an idle channel still requires a wait before transmitting), the contention window (choose a random number of slots to wait), and acknowledgments (a positive ACK plus a time-out timer proves the frame arrived).", sec: "s6" },
    { q: "Distinguish SIFS from DIFS.", a: "SIFS is the short interframe space, used for control and acknowledgment frames inside an existing exchange. DIFS is the longer DCF interframe space, waited out before a station starts a new exchange or sends an RTS. Because SIFS is shorter, a station mid-exchange always wins over one starting fresh.", sec: "s7" },
    { q: "What is the network allocation vector (NAV)?", a: "A timer set from the duration field of an overheard RTS or CTS. It records how long the medium is reserved, and a station checks its NAV before sensing the medium — if the NAV has not expired, it does not even try to sense. This is virtual carrier sensing.", sec: "s7" },
    { q: "Explain the hidden node problem and its standard solution.", a: "Two nodes can each reach the AP but not each other, so both sense an idle medium and transmit simultaneously, colliding at the AP. The solution is the RTS/CTS handshake: the CTS is broadcast by the AP and is heard by the hidden station, which then sets its NAV and defers.", sec: "s8" },
    { q: "How does the exposed node problem differ from the hidden node problem?", a: "Hidden node is a false negative: the medium is busy but the station thinks it is idle, so it transmits and ruins a frame. Exposed node is a false positive: the station hears a transmission that would not have interfered, wrongly defers, and idles the channel unnecessarily.", sec: "s8" },
    { q: "How does the reservation method divide time?", a: "Time is divided into intervals, and a reservation frame precedes the data frames in each interval. With N stations there are exactly N reservation minislots, one per station, and a station claims its own minislot to announce that it has data to send.", sec: "s9" },
    { q: "Distinguish the select function from the poll function in polling.", a: "Select is used when the primary has something to send: it alerts a secondary, gets a ready acknowledgment, sends data, then waits for an ACK. Poll is used to solicit data: the primary asks each secondary in turn, and a secondary replies NAK if it has nothing or sends its data and waits for an ACK.", sec: "s9" },
    { q: "What is the chief drawback of polling, and what is the chief drawback of token passing?", a: "Polling fails entirely if the primary station fails, and it wastes time polling idle secondaries. Token passing requires token management, because a lost or duplicated token must be detected and regenerated or access stops.", sec: "s9" },
    { q: "What is the difference between FDMA and FDM?", a: "FDM is a physical-layer multiplexing technique that uses a physical multiplexer to shift and combine low-bandwidth channels into one high-bandwidth bandpass signal. FDMA is a data-link access method in which each station's own data-link layer drives its physical layer to emit a bandpass signal, with no physical multiplexer involved.", sec: "s10" },
    { q: "What is the main problem with TDMA and how is it addressed?", a: "Synchronization between stations, aggravated by propagation delay, so a guard time is inserted at the start of each slot. Bit-level synchronization is a separate problem, addressed with preamble (synchronization) bits at the head of each slot.", sec: "s10" },
    { q: "How does CDMA encode and decode a bit?", a: "To send bit 1, multiply the station's orthogonal chip sequence by +1; for bit 0 multiply by −1; to stay idle, contribute nothing. The medium carries the chip-by-chip sum. The receiver dots the composite with the target's sequence and divides by the number of chips, giving a positive value for 1 and a negative value for 0.", sec: "s11" },
    { q: "Why must CDMA chip sequences be orthogonal?", a: "Because orthogonality makes the decoder work without coordination: a sequence dotted with itself gives N while two different sequences dot to 0, so every other station's contribution vanishes in the dot product and only the target station's bit survives.", sec: "s11" }
  ],
  quiz: [
    {
      ...
      answer: 1,
      why: "The slides define a link used by nodes or stations in common as a multi-point or broadcast link, and that is exactly what creates the need for MAC protocols.",
      sec: "s1"
    },
    {
      ...
      answer: 2,
      why: "Pure ALOHA never senses the channel, so a frame can be destroyed by a competitor starting anywhere from one frame time before to one frame time after it — a window of 2 × Tfr.",
      sec: "s3"
    },
    {
      ...
      answer: 1,
      why: "Slotted ALOHA follows S = G·e^(−G), which peaks at 1/e ≈ 36.8 percent when the offered load G equals 1 frame per frame time.",
      sec: "s3"
    },
    {
      ...
      answer: 1,
      why: "S = G·e^(−2G) is maximized where its derivative vanishes, at G = 0.5, giving S = 0.5 × e^(−1) ≈ 0.184, or 18.4 percent.",
      sec: "s3"
    },
    {
      ...
      answer: 1,
      why: "Carrier sense is defeated by propagation delay: the first bit takes time to reach every station, so a distant station can find the channel apparently idle and transmit into the incoming frame.",
      sec: "s4"
    },
    {
      ...
      answer: 2,
      why: "1-persistent CSMA senses continuously and pounces the instant the medium becomes free. That wastes no idle time but maximizes the chance that several waiting stations collide.",
      sec: "s4"
    },
    {
      ...
      answer: 2,
      why: "Because a CSMA station senses first, the only window in which it can be fooled is the time for the first bit to cross the medium — a single propagation delay Tp, unrelated to frame length.",
      sec: "s4"
    },
    {
      ...
      answer: 2,
      why: "The range after n collisions is 0 through 2^n − 1. For n = 3 that is 2^3 = 8 slots, so k ranges over 0 through 7.",
      sec: "s5"
    },
    {
      ...
      answer: 1,
      why: "A station must still be transmitting after 2 × Tp, otherwise it will have stopped listening before the collision signal returns. Minimum length equals 2 × Tp × R bits — 512 bits, or 64 bytes, on classic 10 Mbps Ethernet.",
      sec: "s5"
    },
    {
      ...
      answer: 3,
      why: "The jam signal belongs to CSMA/CD, where a colliding station broadcasts noise to alert everyone. CSMA/CA's trio is the interframe space, the contention window, and acknowledgments.",
      sec: "s6"
    },
    {
      ...
      answer: 1,
      why: "The timing hierarchy itself enforces priority. A station resuming an exchange waits only SIFS, so it beats any station that must wait the longer DIFS before starting fresh.",
      sec: "s7"
    },
    {
      ...
      answer: 1,
      why: "An RTS announces how long its sender needs the medium, and the corresponding CTS repeats that duration. Stations that overhear either frame load that duration into their NAV and defer until it expires.",
      sec: "s7"
    },
    {
      ...
      answer: 1,
      why: "The hidden node problem is exactly this asymmetry of radio range: carrier sense cannot help because the interfering station's signal never reaches the sender. The RTS/CTS handshake, with the AP's broadcast CTS, is the standard remedy.",
      sec: "s8"
    },
    {
      ...
      answer: 1,
      why: "The reservation frame carries exactly N minislots when the system has N stations, one in a fixed position for each. A station claims its own minislot to announce that it will send data afterwards.",
      sec: "s9"
    },
    {
      ...
      answer: 1,
      why: "FDM combines low-bandwidth channels onto one high-bandwidth link using a physical multiplexer that shifts each channel's band. FDMA is the MAC-layer access method in which each station's data-link layer drives its own physical layer to emit a bandpass signal.",
      sec: "s10"
    },
    {
      ...
      answer: 1,
      why: "A 1 bit is encoded as +1 times the chip sequence and a 0 bit as −1 times it. Multiplying by −1 flips the sign of every chip, and the decoder recovers a negative value, which it reads as 0.",
      sec: "s11"
    },
    {
      ...
      answer: 1,
      why: "Dotting the composite with station 4's sequence (1, −1, −1, 1) gives 1 − 1 + 1 + 3 = 4. Dividing by the chip count N = 4 gives +1, which decodes to bit 1 — the other stations cancel out because the sequences are orthogonal.",
      sec: "s11"
    }
  ]
});
