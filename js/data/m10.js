...
window.NSCOM_MODULES.push({
  ...
  flashcards: [
    { q: "Who wrote the required paper for this module, and where was it published?", a: "Tanim M. Taher, Matthew J. Misurac, Joseph L. LoCicero and Donald R. Ucci of the Department of Electrical and Computer Engineering, Illinois Institute of Technology, Chicago. 'Microwave Oven Signal Interference Mitigation for Wi-Fi Communication Systems', IEEE CCNC 2008 proceedings.", sec: "s1" },
    { q: "What does the paper call the microwave oven, and why does that label matter?", a: "An unintentional interferer for 2.4 GHz IEEE 802.11 Wi-Fi signals. It matters because an unintentional radiator has no receiver, no MAC layer and no reason to cooperate, so you cannot negotiate with it — you must design around it.", sec: "s1" },
    { q: "What is the ISM band and where did its allocation come from?", a: "Industrial, Scientific and Medical — an unlicensed band. The 2.4 GHz ISM band was allocated at the 1947 ITU Atlantic City conference for industrial, scientific and medical equipment; the bargain is that such equipment may radiate freely provided it accepts interference and causes no harmful interference to licensed services.", sec: "s2" },
    { q: "Which devices does the paper list as operating in the 2.4 GHz band?", a: "IEEE 802.11 Wi-Fi access points, wireless laptops, Bluetooth devices and cordless phones — and, as an unintentional interferer, the residential microwave oven. The paper notes the unlicensed ISM bands are very attractive for consumer applications.", sec: "s2" },
    { q: "Why does microwave heating use roughly 2.45 GHz?", a: "It is dielectric heating of water molecules, which are electric dipoles; a fast-reversing field makes them rotate and collide, producing heat. Roughly 2.45 GHz is industrially chosen partly because it falls in an ISM band, so the appliance needs no special radio licence.", sec: "s4" },
    { q: "What is a magnetron and how does it generate microwaves?", a: "A vacuum tube oscillator: a central cathode and a ring of resonant anode cavities, with an axial magnetic field from permanent magnets. Electrons driven outward are curved into rotating spokes that give energy to the cavity fields, producing continuous oscillation around 2.45 GHz.", sec: "s4" },
    { q: "State the paper's description of the MWO signal.", a: "A wideband signal, up to 60 MHz wide, periodic, and synchronised to the 60 Hz AC line cycle. Its main characteristics are the ON-OFF duty cycle, transients and frequency sweep, and the paper notes the ON cycle is less than half of the 0.017 s 60 Hz period.", sec: "s5" },
    { q: "Why is the MWO emission bursty and periodic?", a: "The magnetron's anode supply is produced by a half-wave rectifier. The tube oscillates only while its anode voltage exceeds the oscillation threshold, so it conducts for less than half of each mains cycle. The result is one burst per half-cycle: an amplitude envelope repeating at 2 × the line frequency (120 Hz on a 60 Hz line).", sec: "s5" },
    { q: "At what times does the paper expect MWO transients?", a: "2 ms before or after the zero voltage crossings of the sinusoidal AC line reference. The threshold detector's output y(t) is compared against that 60 Hz reference, and a match means the cognitive radio records a transient detection.", sec: "s5" },
    { q: "What causes the MWO's frequency sweep, and why does it matter to Wi-Fi?", a: "The magnetron is a free-running oscillator whose frequency is pulled by the load impedance of the cooking cavity, which changes with the food and its heating. The sweep, plus the 50/60 Hz amplitude modulation, spreads the interference as the paper's AM-FM signal, so a 2.46 GHz Wi-Fi channel sees high interference even away from the nominal centre frequency.", sec: "s6" },
    { q: "Why does CSMA fail against a microwave oven?", a: "CSMA with collision avoidance is a mutual agreement — every Wi-Fi node senses the medium, defers when busy and backs off randomly. The paper notes the MWO is oblivious to this type of interference avoidance: it has no carrier sense, no backoff and no MAC, so it radiates regardless of whether a Wi-Fi frame is in progress.", sec: "s7" },
    { q: "What does the paper say follows from high packet drop rates in 802.11?", a: "Wi-Fi connection loss — the paper cites reference [6] on this. High packet drop rates due to interference lead to connection loss, and the paper's mitigation can be applied to minimise packet drop rates due to MWO interference.", sec: "s7" },
    { q: "What spreading code and what rate does the paper's ComBlock testbed use?", a: "The ComBlock transmitter operates at 363 kbps with the 11 chip Barker spreading code, and the paper states the modulated signal's bandwidth is 8 MHz. Data is sent in 128 bit packets, which the ComBlock receiver captures and decodes to obtain the experimental BER.", sec: "s8" },
    { q: "Why is the Barker code good for synchronisation?", a: "Barker codes have a single large autocorrelation peak with very low side lobes — for the length-11 code the side lobes are only ±1 against a peak of 11, keeping false peaks and sidelobe-induced misalignment very unlikely.", sec: "s8" },
    { q: "Name the blocks in the paper's cognitive radio and their jobs.", a: "Baseband converter down-converts the received 2.4 GHz ISM signal; threshold detector senses any signal above the background noise threshold giving y(t); transient detector compares y(t) with the 60 Hz AC line reference; the decision logic declares an MWO after several transients over consecutive line cycles; the transmit controller (50 / 100 %) gates the Wi-Fi transmitter.", sec: "s10" },
    { q: "How does the cognitive radio avoid triggering on its own Wi-Fi traffic?", a: "It does not detect energy in general; it correlates candidate events with the 60 Hz AC line reference and only accepts transients at the expected 2 ms-before-or-after-zero-crossing times. Requiring several consecutive line cycles of matches makes a chance Wi-Fi alignment negligible. The paper says it ignores all Wi-Fi signals and only triggers when a MWO signal is present.", sec: "s11" },
    { q: "What does the transmit controller's '(50 / 100 %)' label mean?", a: "Two states: if no MWO is detected the transmitter operates normally at 100%; if an MWO is detected the transmitter synchronises with the AC line cycle and operates only during the MWO OFF cycles, i.e. roughly 50% of the time.", sec: "s12" },
    { q: "How much data rate does the mitigation cost, and what does it buy?", a: "The paper's tables show 363.3 kbps unmitigated dropping to 181.7 kbps mitigated — a halving. What it buys is the BER falling to 0.000000 for all three ovens, i.e. data packets are reliably transmitted even while the MWO operates.", sec: "s12" },
    { q: "What were the three experimental controls in the paper's testbed?", a: "Three different MWOs were used; two scenarios (no mitigation and mitigated) were run for each; and the receiver was placed equidistant from the Wi-Fi transmitter and the interfering MWO. The Wi-Fi channel tested was 2.46 GHz, inside the MWO's AM-FM region.", sec: "s13" },
    { q: "Give the paper's Case 1 (no mitigation) BERs.", a: "MWO #1: 0.016610 at 363.3 kbps. MWO #2: 0.112900 at 363.3 kbps. MWO #3: 0.007315 at 363.3 kbps. The results vary depending on the MWO used.", sec: "s15" },
    { q: "Give the paper's Case 2 (mitigated) BERs and rate.", a: "All three MWOs gave BER 0.000000 at a data rate of 181.7 kbps — the measured zero BER obtained in the interference mitigated case.", sec: "s15" },
    { q: "Why does the paper say the unmitigated system's actual throughput is lower despite its higher data rate?", a: "In the unmitigated case the data rate stays at 100% but the BER is much higher and the ComBlock data packets are corrupted. So the actual throughput of the experimental Wi-Fi system is much less than the mitigated case even though the data transmission rate is higher.", sec: "s15" },
    { q: "What caveat does the paper attach to its BER tables?", a: "Because performance varies considerably if the distances between receiver, transmitter and MWO change, Tables 1 and 2 are meant only for comparative purposes. They demonstrate performance with or without mitigation and are not absolute records. It also notes lower MWO-to-receiver distances will elevate BER.", sec: "s15" },
    { q: "State the paper's conclusion in one sentence.", a: "A cognitive radio experimental technique was implemented that successfully mitigated interference on Wi-Fi communications caused by MWO signals, and this system provides reliable data transmission when a MWO is operating in proximity to a Wi-Fi system.", sec: "s16" },
    { q: "What does the paper say about the practicality of the technique?", a: "The method is practically realisable on consumer access points and other Wi-Fi devices. The technique is described as applicable to IEEE 802.11 Wi-Fi systems in general, not only to the ComBlock testbed.", sec: "s16" },
    { q: "What is a measured BER of exactly zero really telling you?", a: "That no bit errors were observed during the test — an upper bound rather than proof that errors are impossible. The honest statement is that the true BER is below roughly one divided by the total bits observed in the run.", sec: "s14" },
  ],
  quiz: [
    {
      q: "According to the paper, what kind of interferer is a residential microwave oven?",
      choices: [
        "An intentional radiator that shares the band under a licence",
        "An unintentional interferer for 2.4 GHz IEEE 802.11 Wi-Fi signals",
        "A licensed primary user of the 2.4 GHz ISM band",
        "A narrowband interferer confined to a single 20 MHz Wi-Fi channel",
      ],
      answer: 1,
      why: "The paper's abstract calls the MWO a popular appliance that acts as an unintentional interferer for 2.4 GHz IEEE 802.11 Wi-Fi communication signals. It also describes its emission as wideband, up to 60 MHz, not narrowband.",
      sec: "s1",
    },
    {
      q: "Which group of devices does the paper name as sharing the 2.4 GHz band?",
      choices: [
        "Broadcast television, FM radio and amateur radio",
        "Wi-Fi access points, wireless laptops, Bluetooth devices and cordless phones",
        "Only IEEE 802.11 access points and their associated clients",
        "Satellite uplinks, radar and licensed microwave backhaul",
      ],
      answer: 1,
      why: "The paper's introduction lists IEEE 802.11 Wi-Fi access points, wireless laptops, Bluetooth devices and cordless phones as operating in the 2.4 GHz band — an unlicensed ISM band that is very attractive for consumer applications.",
      sec: "s2",
    },
    {
      q: "Why is the MWO's interference bursty and periodic rather than continuous?",
      choices: [
        "Because the turntable periodically blocks the waveguide",
        "Because the door seal opens and closes during cooking",
        "Because a half-wave rectified anode supply lets the magnetron conduct for less than half of each mains cycle",
        "Because the magnetron is switched off every time the control panel timer updates",
      ],
      answer: 2,
      why: "The half-wave supply lets the tube oscillate only while its anode voltage exceeds the oscillation threshold, so it conducts for under half a mains cycle. The envelope therefore repeats at twice the line frequency — the reason the paper synchronises the mitigation to the 60 Hz AC line.",
      sec: "s5",
    },
    {
      q: "The paper reports that the MWO's ON cycle is less than half of which period?",
      choices: [
        "0.017 s, the 60 Hz AC line period",
        "0.0001 s, one Wi-Fi chip duration",
        "1 s, the access point beacon interval",
        "0.017 ms, the MWO's switching interval",
      ],
      answer: 0,
      why: "The paper states that the ON cycle is less than half of the 0.017 s 60 Hz period. That is 1/60 s ≈ 0.0167 s, the period of the 60 Hz AC line to which the MWO emission is synchronised.",
      sec: "s5",
    },
    {
      q: "Where does the paper expect MWO transients to occur in time?",
      choices: [
        "Randomly, with no relation to any system clock",
        "Exactly at the peak of each AC line half-cycle",
        "2 ms before or after the zero voltage crossings of the AC line reference",
        "At multiples of the 8 MHz Wi-Fi symbol rate",
      ],
      answer: 2,
      why: "The transient detector compares y(t) with the AC line reference, and the paper states the expected transient time locations are 2 ms before or after the zero voltage crossings of the sinusoidal AC line reference.",
      sec: "s5",
    },
    {
      q: "What does the paper say is necessary for successful interference mitigation?",
      choices: [
        "Raising the Wi-Fi transmit power above the MWO's leakage",
        "Detecting the presence of MWO interference signals and synchronising the data transmitter with the MWO's ON-OFF cycles",
        "Adding a narrowband notch filter at 2.45 GHz in the receiver front end",
        "Moving the access point into a metal enclosure",
      ],
      answer: 1,
      why: "The paper states that for successful interference mitigation it is necessary to detect the presence of MWO interference signals and synchronise the data transmitter with the MWO's ON-OFF cycles. That is exactly what the experimental cognitive radio does.",
      sec: "s9",
    },
    {
      q: "Why does the paper say CSMA cannot solve MWO interference?",
      choices: [
        "Because CSMA is too slow for a 2.4 GHz channel",
        "Because the MWO is oblivious to this type of interference avoidance",
        "Because CSMA requires a licence in the ISM band",
        "Because CSMA only works in 5 GHz bands",
      ],
      answer: 1,
      why: "The paper acknowledges CSMA is effective in Wi-Fi collision avoidance but points out the MWO is oblivious to this type of interference avoidance — it has no carrier sense, no backoff and no MAC layer, so it cannot be negotiated with.",
      sec: "s7",
    },
    {
      q: "What does the cognitive radio do once it concludes a MWO interference signal is present?",
      choices: [
        "It raises its transmit power and repeats each packet three times",
        "It instructs the Wi-Fi transmitter to synchronise with the AC line cycle and operate only during the MWO OFF cycles",
        "It switches off the Wi-Fi radio entirely until the MWO stops",
        "It retunes the access point to 2.46 GHz to avoid the oven",
      ],
      answer: 1,
      why: "The transmit controller instructs the Wi-Fi transmitter to synchronise with the AC line cycle and operate only during the MWO OFF cycles. If no MWO is detected it instructs the transmitter to operate normally.",
      sec: "s10",
    },
    {
      q: "What did the experimental ComBlock transmitter use for spreading?",
      choices: [
        "A 64 chip Walsh-Hadamard code",
        "The 11 chip Barker spreading code at 363 kbps",
        "Frequency hopping over 79 channels",
        "No spreading at all, using a simple BPSK carrier",
      ],
      answer: 1,
      why: "The paper's testbed section states the ComBlock transmitter operated at 363 kbps with the 11 chip Barker spreading code, and that the modulated signal's bandwidth is 8 MHz.",
      sec: "s13",
    },
    {
      ...
      answer: 1,
      why: "The paper states the modulated signal's bandwidth is 8 MHz. The 60 MHz figure in the paper refers to the width of the MWO's interference, not to the Wi-Fi signal.",
      sec: "s13",
    },
    {
      q: "How did the paper obtain its experimental bit error rate?",
      choices: [
        "By simulating the MWO in software with no physical hardware",
        "By decoding the transmitted 128 bit ComBlock packets at the receiver and comparing them with what was sent",
        "By measuring the received signal power with a spectrum analyser",
        "By reading the error counters of a commercial 802.11 access point",
      ],
      answer: 1,
      why: "The paper says the data is transmitted in 128 bit packets by the ComBlock, and the ComBlock receiver captures and decodes those packets, which are used to obtain the experimental BER.",
      sec: "s13",
    },
    {
      ...
      answer: 3,
      why: "Table 1 of the paper records BER 0.112900 at 363.3 kbps for MWO #2 — the worst of the three ovens. 0.016610 is MWO #1 and 0.007315 is MWO #3.",
      sec: "s15",
    },
    {
      q: "What BER did the paper measure for all three MWOs once mitigation was active, and at what data rate?",
      choices: [
        "BER 0.000000 at 181.7 kbps",
        "BER 0.000000 at 363.3 kbps",
        "BER 0.001000 at 181.7 kbps",
        "BER 0.007315 at 363.3 kbps",
      ],
      answer: 0,
      why: "The paper's Table 2 shows the measured zero BER for all three ovens in the interference mitigated case, and the data rate is 181.7 kbps — a drop to 50% of the 363.3 kbps unmitigated rate.",
      sec: "s15",
    },
    {
      q: "What is the paper's argument about data rate versus throughput?",
      choices: [
        "Higher data rate always means higher throughput, so no mitigation should be used",
        "The unmitigated case keeps 100% data rate but corrupts packets, so its actual throughput is much less than the mitigated case",
        "Data rate and throughput are identical in 802.11 systems",
        "The mitigated case has both a higher data rate and a higher throughput",
      ],
      answer: 1,
      why: "The paper states that without mitigation the data rate remains at 100% but the BER is much higher and the ComBlock data packets are corrupted, so the actual throughput is much less than the mitigated case even though the data transmission rate is higher.",
      sec: "s15",
    },
    {
      q: "What caveat does the paper attach to its two BER tables?",
      choices: [
        "They were obtained entirely by simulation rather than measurement",
        "Because performance varies considerably with the distances involved, the tables are meant only for comparative purposes",
        "They apply only to ComBlock hardware and never to IEEE 802.11",
        "The BER values are estimates, since no error counting was performed",
      ],
      answer: 1,
      why: "The paper notes that performance varies considerably if the distances between the receiver, transmitter and MWO are changed, so Tables 1 and 2 are meant only for comparative purposes — they demonstrate performance with or without mitigation rather than absolute values.",
      sec: "s15",
    },
    {
      q: "What is the paper's overall conclusion?",
      choices: [
        "MWO interference cannot be mitigated in the 2.4 GHz band",
        "A cognitive radio experimental technique successfully mitigated MWO interference on Wi-Fi, providing reliable data transmission when an MWO operates in proximity",
        "802.11 should abandon the 2.4 GHz band entirely",
        "Increasing Wi-Fi transmit power is the most effective mitigation",
      ],
      answer: 1,
      why: "The paper's conclusion section states a cognitive radio experimental technique was implemented that successfully mitigated interference on Wi-Fi communications caused by MWO signals, providing reliable data transmission when a MWO operates near a Wi-Fi system.",
      sec: "s16",
    },
  ],
});
